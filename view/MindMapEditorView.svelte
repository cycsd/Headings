<script lang="ts">
	...
	let root: Root = {
		id: "root",
		fileName: plugin.currentFile!.name,
	};
	let blocks = $derived.by(() => {
		if (!cache || !plugin.currentMarkdownDoc) return [];
		const contents = parseCacheMetadata2Content(
			cache,
			plugin.currentMarkdownDoc,
		);
	...
	export function refreshCache(newCache: CachedMetadata) {
		cache = newCache;
	}
	function logEditor() {
